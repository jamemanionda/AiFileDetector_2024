...
class ForensicsDataPreprocessing(QMainWindow, form_class):  # QMainWindow, form_class
    ...
    def lstm(self, df):
        ...
        model = Sequential()
        model.add(Bidirectional(LSTM(15, return_sequences=True, kernel_regularizer=l2(0.01)),
                                input_shape=(1, X_train_scaled.shape[2])))
        model.add(BatchNormalization())
        model.add(Dropout(0.2))
        model.add(Bidirectional(LSTM(15, kernel_regularizer=l2(0.01))))
    ...
    def predict_data(self, df):
        ...
        if self.index == 0 or self.index == 2 or self.index == 3 or self.index == 4:
            ...
            X_train_scaled = X_train_scaled.reshape((X_train_scaled.shape[0], 1, X_train_scaled.shape[1]))
            y_pred_new = self.model.predict(X_train_scaled)
